...
export const appRouter = router({
  ...
  dashboard: router({ summary: protectedProcedure.query(({ ctx }) => getDashboardSummary(ctx.user.id)), redeemKey: protectedProcedure.input(z.object({ key: z.string().trim().toUpperCase().regex(/^CHROMA-[A-Z0-9]{6}-[A-Z0-9]{6}-[A-Z0-9]{6}$/) })).mutation(async ({ ctx, input }) => { const result = await redeemSubscriptionKey({ key: input.key, userId: ctx.user.id }); if (!result) throw new TRPCError({ code: "BAD_REQUEST", message: "Ключ недействителен, исчерпал лимит активаций, истёк или отозван." }); await createAuditLog({ userId: ctx.user.id, action: "SUBSCRIPTION_KEY_REDEEMED", metadata: { subscriptionId: result.subscriptionId, plan: result.plan.slug, durationDays: result.durationDays, usedActivations: result.usedActivations, maxActivations: result.maxActivations } }); return { success: true, plan: result.plan.name, durationDays: result.durationDays, usedActivations: result.usedActivations, maxActivations: result.maxActivations } as const; }) }),
  ...
  admin: router({
    ...
    createSubscriptionKey: adminProcedure.input(z.object({ planId: z.number().int().positive(), maxActivations: z.number().int().min(1).max(100000), expiresAt: z.coerce.date() })).mutation(async ({ ctx, input }) => { if (input.expiresAt <= new Date()) throw new TRPCError({ code: "BAD_REQUEST", message: "Дата окончания должна быть в будущем." }); const result = await createSubscriptionKey({ ...input, createdByUserId: ctx.user.id }); if (!result) throw new TRPCError({ code: "BAD_REQUEST", message: "Нельзя создать ключ для FREE или неактивного тарифа." }); await createAuditLog({ userId: ctx.user.id, action: "ADMIN_SUBSCRIPTION_KEY_CREATED", metadata: { keyId: result.id, planId: input.planId, maxActivations: input.maxActivations, expiresAt: input.expiresAt.toISOString() } }); return result; }),
