...
import { getSupabaseAdminStats, getSupabaseAdminSubscriptionData, getSupabaseAdminUsers, getSupabaseDashboard, getSupabasePlans, getSupabaseVersions, getSupabaseVisuals, updateSupabaseRole } from "./supabaseData";
...
export const appRouter = router({
  ...
  admin: router({
    ...
    updateRole: adminProcedure.input(z.object({ openId: z.string().uuid(), role: z.enum(["user", "developer", "admin", "support", "media", "moderator"]) })).mutation(async ({ ctx, input }) => { if (input.role === "developer" && ctx.user.role !== "developer") throw new TRPCError({ code: "FORBIDDEN", message: "Only Developer can grant Developer role." }); const ok = await updateSupabaseRole(input.openId, input.role); if (!ok) throw new TRPCError({ code: "NOT_FOUND", message: "User not found" }); await createAuditLog({ userId: ctx.user.id, action: "ADMIN_ROLE_UPDATED", metadata: { targetOpenId: input.openId, role: input.role } }); return { success: true }; }),
